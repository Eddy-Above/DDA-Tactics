...
async function loadDigimon() {
  ...
  Object.assign(form, {
    ...
    type: digimon.type,
    size: digimon.size || 'medium',
    baseStats: { ...digimon.baseStats },
    bonusStats: { ...(digimon.bonusStats || { accuracy: 0, damage: 0, dodge: 0, armor: 0, health: 0 }) },
...
// Has linked evolutions
const hasLinkedEvolutions = computed(() => {
  return (form.evolvesFromId !== null && form.evolvesFromId !== undefined) ||
         (form.evolutionPathIds && form.evolutionPathIds.length > 0)
})
</script>
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="mb-8">
      <NuxtLink :to="`/campaigns/${campaignId}/player/${tamerId}`" class="text-digimon-dark-400 hover:text-white text-sm mb-2 inline-block">
        &larr; Back to Dashboard
      </NuxtLink>
      <h1 class="font-display text-3xl font-bold text-white">Edit Partner Digimon</h1>
    </div>

    <div v-if="!initialDigimon && !loadError" class="text-center py-12">
    ...
    <div v-else-if="loadError" class="text-center py-12">
      ...
      <NuxtLink
        :to="`/campaigns/${campaignId}/player/${tamerId}`"
          ...
          <div class="flex flex-col items-center">
            ...
            <template v-if="evolutionChain.descendantsTree.length > 0">
              <template v-if="evolutionChain.descendantsTree.length === 1">
                <div class="w-px h-4 bg-digimon-dark-500"></div>
                <EvolutionTreeBranch :node="evolutionChain.descendantsTree[0]" :link-base="`/campaigns/${campaignId}/player/${tamerId}/digimon`" />
              </template>
              <template v-else>
                <div class="flex flex-col items-center">
                  ...
                  <div class="flex">
                    ...
                    >
                      ...
                      <EvolutionTreeBranch :node="node" :link-base="`/campaigns/${campaignId}/player/${tamerId}/digimon`" />
                    </div>
                  </div>
                </div>
              </template>
