...
// so the result modal stays hidden until every target in the group has resolved
const attackResultGroupTotals = ref<Record<string, number>>({})
...
const accuracyReviewContext = ref<{
  ...
  clashAreaAttack?: { controllerId: string; opponentId: string } | null
} | null>(null)
...
function reconstructAttackResults(encounter: Encounter, fetchedTamer: Tamer) {
  ...
  for (const resp of responses) {
    ...
    if (pendingAttacks.value.some((pa) => pa.participantId === resp.attackerParticipantId && pa.targetParticipantId === resp.participantId)) continue
    ...
    const syntheticPendingAttack = {
      ...
      targetParticipantId: resp.participantId,
    ...
    for (let i = pendingAttacks.value.length - 1; i >= 0; i--) {
      ...
      if (!matchingResponse) {
        ...
        if (now - pendingAttack.timestamp > 300000) {
          ...
          if (pendingAttack.groupId && attackResultGroupTotals.value[pendingAttack.groupId] != null) {
            attackResultGroupTotals.value[pendingAttack.groupId]--
            maybeShowGroupModal(pendingAttack.groupId)
          }
        }
        continue
      }
...
const dodgeDicePool = computed(() => {
  ...
  // Attacked from outside the clash: the target cannot dodge at all.
  if (currentDodgeRequest.value?.data?.cannotDodge) {
    return 0
  }
...
const clashAttackParticipant = computed(() =>
  (activeEncounter.value?.participants as CombatParticipant[] | undefined)?.find(x => x.id === clashAttackActorId.value)
)
...
async function confirmAreaAttack(targets: CombatParticipant[], areaShapeData?: AreaShapeData | null) {
  ...
  if (clashCtx) {
    const allParts = (activeEncounter.value?.participants as CombatParticipant[]) || []
  ...
  try {
    if (ctx.areaTargets) {
      ...
      const result = await performAttack(
        ...
        { targetIds: targets.map(t => t.id), areaShapeData: ctx.areaShapeData, clashNoInterceptTargetId: ctx.clashAreaAttack?.opponentId }
      )

      // Pass clash attack ends the clash once the attack is submitted.
      if (ctx.clashAreaAttack) await executeClashAction(ctx.clashAreaAttack.controllerId, 'end')
...
function showAttackResult(
  ...
  const matchingLogEntry = [...battleLog].reverse().find(
    (entry: any) =>
      ...
      entry.attackerParticipantId === pendingAttack.participantId &&
      entry.actorId === dodgeRequest.targetParticipantId &&
      entry.hit !== undefined
  )
  ...
  const resolvedGroupId = groupId ?? dodgeResponse.id
  ...
  // Show modal only once every target in the group has resolved
  maybeShowGroupModal(resolvedGroupId)
  console.log('[ATTACK RESULT] Modal flag now:', showAttackResultModal.value)
}
...
function showAttackResultFromBattleLog(
  ...
  const resolvedGroupId = groupId ?? `log-${logEntry.id}`
...
// Flips the result modal on once every target in the AoE group has a queued
// result. Single-target attacks have no tracked total and show immediately.
function maybeShowGroupModal(groupId?: string) {
  if (!groupId) {
    showAttackResultModal.value = true
    return
  }
  const total = attackResultGroupTotals.value[groupId]
  const count = attackResultQueue.value.filter(r => r.groupId === groupId).length
...
async function closeAttackResultModal() {
  ...
  if (closedGroupId) {
    delete attackResultGroupTotals.value[closedGroupId]
  }
...
// Clear selection
const selectedTamerId = useCookie<string | null>(`player-tamer-id-${campaignId.value}`)
function switchCharacter() {
  selectedTamerId.value = null
  navigateTo(`/campaigns/${campaignId.value}/player`)
}
...
// Clash Attack: open the melee-only attack picker for the controller. Picking an attack rolls
// accuracy and submits to clash-action — see confirmClashAttack.
function startClashAttack(participantId: string) {
  if (!activeEncounter.value) return
  const participant = (activeEncounter.value.participants as CombatParticipant[]).find(p => p.id === participantId)
  if (!(participant as any)?.clash?.isController) return
...
const mapStanceParticipant = computed(() =>
  mapStanceDigimonParticipantId.value
    ? (activeEncounter.value?.participants as CombatParticipant[])?.find(x => x.id === mapStanceDigimonParticipantId.value) ?? null
    : null
)
...
<template>
          ...
          <div v-if="showMapView && activeEncounter" class="fixed inset-0 z-40 bg-digimon-dark-900" style="top:56px;left:0;right:0;bottom:0;">
            <EncounterMap
              ...
              @player-action="(id, action) => {
                ...
                else if (action === 'clash-attack') startClashAttack(id)
            ...
            <div
              v-if="mapStanceParticipant"
              class="fixed z-50 bg-digimon-dark-800 border border-digimon-dark-600 rounded-xl p-4 shadow-xl"
              style="bottom: 120px; left: 50%; transform: translateX(-50%); min-width: 280px; max-width: 380px;"
            >
              <div class="text-sm text-digimon-dark-400 mb-3 text-center">Select Stance (1 Simple)</div>
              <div class="flex flex-wrap gap-2 justify-center">
                <button
                  v-for="stance in (['neutral', 'defensive', 'offensive', 'sniper', 'brave'] as const)"
                  :key="stance"
                  class="px-3 py-2 rounded text-sm capitalize bg-digimon-dark-700 text-digimon-dark-200 hover:bg-digimon-dark-600"
                  @click="changePlayerStance(mapStanceParticipant!, stance); mapStanceDigimonParticipantId = null"
                >{{ stance }}</button>
              </div>
              <button class="mt-3 w-full text-xs text-digimon-dark-500 hover:text-white" @click="mapStanceDigimonParticipantId = null">Cancel</button>
            </div>
            <!-- Floating digivolve picker (shown when player clicks Digivolve from map radial) -->
            <div
              v-if="mapDigivolveParticipant"
              class="fixed z-50 bg-digimon-dark-800 border border-digimon-dark-600 rounded-xl p-4 shadow-xl"
              style="bottom: 120px; left: 50%; transform: translateX(-50%); min-width: 280px; max-width: 380px;"
            >
              <div class="text-sm text-digimon-dark-400 mb-3 text-center">Digivolve / Devolve</div>
              <div class="flex flex-col gap-2">
                <button
                  v-for="target in getParticipantEvolutionOptions(mapDigivolveParticipant!).evolveTargets"
                  :key="target.chainIndex"
                  class="px-3 py-2 rounded text-sm text-left bg-green-900/50 text-green-300 hover:bg-green-900/70"
                  @click="handleDigivolve(mapDigivolveParticipant!, target.chainIndex); mapDigivolveDigimonParticipantId = null"
                >Digivolve → {{ target.species }}</button>
                <button
                  v-if="getParticipantEvolutionOptions(mapDigivolveParticipant!).canDevolve"
                  class="px-3 py-2 rounded text-sm text-left bg-amber-900/50 text-amber-300 hover:bg-amber-900/70"
                  @click="handleDigivolve(mapDigivolveParticipant!, getParticipantEvolutionOptions(mapDigivolveParticipant!).devolveTarget.chainIndex); mapDigivolveDigimonParticipantId = null"
                >Devolve → {{ getParticipantEvolutionOptions(mapDigivolveParticipant!).devolveTarget?.species }}</button>
                <div
                  v-if="!getParticipantEvolutionOptions(mapDigivolveParticipant!).evolveTargets.length && !getParticipantEvolutionOptions(mapDigivolveParticipant!).canDevolve"
                  class="text-center text-digimon-dark-400 text-sm py-2"
                  ...
                  <template v-if="(participant as any).clash">
                    <template v-if="(participant as any).clash.isController && !(participant as any).clash.clashCheckNeeded">
                      <button
                        ...
                        @click="startClashAttack(participant.id)"
                      >
                        Clash Attack (2)
                      </button>
                      <button
              ...
              <div class="flex items-start gap-4 mb-4">
                ...
                <div>
                  <div class="flex items-center gap-3">
                    ...
                    <NuxtLink
                      :to="`/campaigns/${campaignId}/player/${tamerId}/edit`"
  ...
  <Teleport to="body">
    ...
    >
      <div class="bg-digimon-dark-800 rounded-xl p-6 w-full max-w-md border-2 border-red-500">
        ...
        <div v-if="currentDodgeRequest?.data?.cannotDodge" class="mb-4 p-3 bg-red-900/40 rounded-lg border border-red-600">
          <p class="text-red-300 text-sm font-medium">
            You're locked in a clash — you cannot dodge attacks from outside it. This attack hits automatically.
          </p>
        </div>
  ...
  <Teleport to="body">
    ...
    >
      <div class="bg-digimon-dark-800 rounded-xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto border-2 border-red-600">
        <h2 class="font-display text-xl font-semibold text-white mb-1">Clash Attack</h2>
    ...
    >
      <div class="bg-digimon-dark-800 rounded-xl p-6 w-full max-w-md border-2 border-digimon-orange-500">
        ...
        <div class="flex gap-3">
          ...
          <button
            v-else
            @click="confirmAreaAttack(getEnemyTargets().filter(t => selectedTargetIds.includes(t.id)))"
          ...
          <button
            @click="showTargetSelector = false; selectedAttack = null; selectedTargetId = null; selectedTargetIds = []; clashInitiateRollResult = null"
  ...
  <Teleport to="body">
    <div
      ...
      class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4"
    >
      <div class="bg-digimon-dark-800 rounded-xl p-6 w-full max-w-2xl border-2 border-orange-500 max-h-[85vh] overflow-y-auto">
        ...
        <div
          ...
          class="mb-2 p-3 bg-digimon-dark-700 rounded-lg border-2"
          :class="[
        ...
        >
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-semibold" :class="[
              ...
              {{ result.hit ? 'HIT!' : 'MISS!' }}<template v-if="attackResultGroup.length > 1"> vs {{ result.targetName }}</template>
            </h3>
            <div class="text-digimon-dark-400">
              Net Successes:
              <span :class="[
          ...
          <div v-if="result.defeated" class="mt-4 pt-4 border-t border-digimon-dark-600 text-center">
            <span class="text-yellow-400 font-bold text-base tracking-wide">DEFEATED!</span>
    ...
    >
      <div class="bg-digimon-dark-800 rounded-xl p-6 w-full max-w-md border-2 border-purple-500">
        ...
        <div class="mb-4 p-3 bg-purple-900/20 rounded-lg">
          <p class="text-white text-sm">
            DC <span class="font-semibold">{{ DIGIVOLVE_WILLPOWER_DC[campaignLevel] }}</span>
        ...
        <div class="mb-4">
          ...
          >
            ...
            <div class="text-sm" :class="willpowerRollResult.total >= DIGIVOLVE_WILLPOWER_DC[campaignLevel] ? 'text-green-400' : 'text-red-400'">
              {{ willpowerRollResult.total >= DIGIVOLVE_WILLPOWER_DC[campaignLevel] ? 'SUCCESS' : 'FAILURE' }}
              <span class="text-digimon-dark-400 ml-1">(vs DC {{ DIGIVOLVE_WILLPOWER_DC[campaignLevel] }})</span>
            </div>
          </div>
        </div>
