...
// Cached evolution chain (computed once, not 5x in template)
...
async function handleSubmit() {
  ...
  router.push(`/campaigns/${campaignId.value}/library/digimon`)
}
...
async function handleCopy() {
  ...
  if (newDigimon) {
    router.push(`/campaigns/${campaignId.value}/library/digimon/${newDigimon.id}`)
  }
  copying.value = false
}
</script>
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="mb-8">
      <NuxtLink :to="`/campaigns/${campaignId}/library/digimon`" class="text-digimon-dark-400 hover:text-white text-sm mb-2 inline-block">
    ...
    <div v-else-if="!digimon" class="text-center py-12">
      ...
      <NuxtLink
        :to="`/campaigns/${campaignId}/library/digimon`"
        ...
        <div v-if="evolutionChain" class="mt-6 pt-4 border-t border-digimon-dark-600">
          ...
          <div class="flex flex-col items-center">
            ...
            <template v-for="(ancestor, index) in evolutionChain.ancestors" :key="ancestor.id">
              <NuxtLink
                :to="`/campaigns/${campaignId}/library/digimon/${ancestor.id}`"
