...
export default defineEventHandler(async (event) => {
  ...
  let tamerAttributes = tamer.attributes
  let tamerSkills = tamer.skills
  let tamerXpBonuses = tamer.xpBonuses
  ...
  if (tamer.campaignId) {
    ...
    if (campaign) {
      ...
      const rules = campaign.rulesSettings || {}
  ...
  if (usageLimit === 'per-day') {
    const usedPerDay = tamer.usedPerDaySkillOrders || []
  ...
  if (usageLimit === 'per-day') {
    ...
    await db.update(tamers).set({
      usedPerDaySkillOrders: [...usedPerDay, body.orderName],
      updatedAt: new Date(),
