...
flowchart TD
    ...
    ServerUtils["server/utils/\napplyEffect, resolveNpcAttack, triggerCounterattack, resolveSupportAttack, resolveAreaIntercedeGroup, computeAttackDamage, encounterPayload, id, password, participantName, mapMovement, gridDistance, areaShapes, encounterRoom"]
    DB["server/db/\nindex.ts + schema.ts"]
    Postgres[("PostgreSQL")]
