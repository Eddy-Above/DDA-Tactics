...
| GET | `/api/encounters/[id]` | `encounters/[id].get.ts` | Full encounter with participants, log; `participantPositions`/`destructibleStates` are served live from the in-memory `RoomState` (`encounterRoom.ts`), not the DB row |
...
| WS | `/api/encounters/[id]/ws` | `encounters/[id]/ws.ts` | WebSocket: sole live-sync channel for `participantPositions`/`destructibleStates`, backed by per-encounter `RoomState` (`server/utils/encounterRoom.ts`). On connect, sends `full-state` (`participantPositions`, `destructibleStates`, `version`). Messages: `unit-moved`/`structure-damaged` (server applies to RoomState, stamps `version`, broadcasts), `position-patch`, `map-edited`, `door-toggled`, `element-painted` (pass-through broadcast), `full-state`, `encounter-updated` (`{ type: 'encounter-updated', encounterId }` — generic "encounter JSON changed, refetch" push; broadcast via `notifyEncounterUpdated()`, fired automatically by `server/middleware/encounter-sync.ts` after any successful POST to `/api/encounters/[id]/actions/*`). RoomState debounce-persists to DB (1500ms) for durability only |
...
| `intercede-offer` | `intercede-offer.post.ts` | `attackId`, `targetId` (single) or `targetIds[]` (area), roll data, `areaShapeData?: AreaShapeData \| null` (area only) | Single-target: creates one intercede-offer per eligible tamer + GM. Support attacks go through intercede same as damage attacks — only exception is single-target melee self-buff (attacker===target), which resolves immediately. Area attack: deducts actions once, NPC targets auto-resolved via support or damage resolvers, creates ONE request per eligible tamer with `data.areaTargetIds` + `isAreaAttack: true`; excludes tamers/partners who are themselves targets. Each request's `data` includes `tamerCanReach`/`digimonCanReach` booleans (per-character spatial eligibility, default `true` with no map) so the player UI can hide an "Intercede with..." option for whichever character can't reach the intercede position. `body.areaShapeData` (from `MapCanvas.vue`'s `area-attack-confirmed` emit) is persisted verbatim as `data.areaShapeData` on every per-tamer `intercede-offer` request AND on the `intercede-group-state` request — the player UI uses it to compute AoE cells client-side (e.g. to offer "Throw `{ally}` Out of the Blast" only when an adjacent ally is inside the blast), and `intercede-claim.post.ts` re-derives `excludeCells` from it for throw-claim validation. Fallback when no eligible interceptors exist (single-target or per-AOE-target): a player target hit by a positive support effect (buff) resolves via resolvePositiveAuto/resolvePositiveHealth (health-roll) instead of a dodge-roll; negative support/damage attacks still create a dodge-roll |
| `intercede-claim` | `intercede-claim.post.ts` | `requestId`, `interceptorParticipantId`, `chosenTargetId?` (area only), `isThrowClaim?: boolean`, `throwAllyLandingPos?: Vec3` | Player steps in to take hit for ally; area attacks: 409 if target already claimed, strips chosen target from other group requests, creates dodge-rolls for any uncovered remaining targets. When `isThrowClaim` is true (the "Throw `{ally}` Out of the Blast" option, Part C): validates `effectiveTargetId` (the ally being thrown) is adjacent to the interceptor; recomputes AoE cells from the persisted `data.areaShapeData` (Part D) as `excludeCells`; resolves the interceptor's `actorBodyStat` → `maxDistance`; validates `throwAllyLandingPos` via `findThrowLandingCell(allyPos, throwAllyLandingPos, maxDistance, allyDims, map, occupiedSet, excludeCells)` — 400 if missing/invalid; applies fall damage to the ally if the landing cell is elevated; updates `participantPositions[effectiveTargetId]`. The resulting `AreaAttackClaim` carries `isThrowClaim: true`, which causes `resolveAreaIntercedeGroup.ts` to skip the rescued ally's `dodgePenalty` increment (they were physically removed, not hit); the interceptor still takes the AoE hit at 0 dodge via the existing full-swap branch |
| `intercede-skip` | `intercede-skip.post.ts` | `requestId`, `optOut?` | Player declines intercede; area attacks (via `resolveAreaIntercedeGroup`): creates dodge-rolls for all targets not covered by remaining requests; opt-out adds all areaTargetIds to intercedeOptOuts; NPC fallback uses support resolver (resolvePositiveAuto/resolvePositiveHealth/resolveNegativeSupportNpc) when `isSupportAttack` is true. For both single-target and area fallbacks, an uncovered player target hit by a positive support effect (buff) resolves via resolvePositiveAuto/resolvePositiveHealth (health-roll) instead of a dodge-roll; negative support/damage attacks still create a dodge-roll |
...
| Throw-aim refs | ~88 | `encounterMapRef` (template ref to `EncounterMap`/`MapCanvas` for `startThrowAim`/`cancelThrowAim` etc.), `throwAimControllerId` (Clash Throw aiming, Part A), `throwAllyAimContext` (Intercede "Throw Ally Out of the Blast" aiming, Part C) |
...
| handleGmIntercedeClaim(requestId, interceptorId, throwOptions?) | ~999 | GM steps interceptor in front; `throwOptions` (`{ isThrowClaim, throwAllyLandingPos }`) forwarded to `intercede-claim` for "Throw Ally Out of the Blast" (Part C) |
...
| handleInitiateClash / handleClashCheck / executeClashAction(participantId, actionType, options?) / handleBreakClash | ~2440–2507 | `executeClashAction` now accepts `actionType: 'attack'\|'end'\|'pin'\|'throw'` and an optional `options: { landingPos?: Vec3 }` (Part A) |
...
| isAdjacentToChosenAlly / handleThrowAllyClick / cancelThrowAllyAim / onThrowAllyLandingSelected | ~? | Intercede "Throw Ally Out of the Blast" aiming (Part C): gates the option to adjacent allies inside the AoE, enters/exits ally landing-cell-picking mode, calls `handleGmIntercedeClaim(requestId, interceptorId, { isThrowClaim: true, throwAllyLandingPos })` on selection |
...
| — select-interceptor view | ~4458 | handleGmIntercedeClaim buttons; new "Throw `{ally}` Out of the Blast" button (`handleThrowAllyClick`) shown only when `isAdjacentToChosenAlly` and `data.areaShapeData` is present (Part C/D) |
...
|---|---|---|---|
| `AttackRiderOptions` | `components/AttackRiderOptions.vue` | Bolster Attack / Lifesteal Complex Action / Huge Power (+ Rank 2) toggle UI for a selected attack; shared by the card-view target-selector modal and the map-view floating rider-options panel on both player and GM pages | `selectedAttack`, `round`, `eddySoulRules?`, `canBolster`, `hugePower: {rank1, rank2}`; `v-model`s: `bolsterAttackEnabled`, `bolsterAttackType`, `lifestealComplexEnabled`, `hugePowerEnabled`, `hugePowerRank2Enabled` |
...
flowchart TD
    ...
    ServerUtils["server/utils/\napplyEffect, resolveNpcAttack, triggerCounterattack, resolveSupportAttack, resolveAreaIntercedeGroup, computeAttackDamage, id, password, participantName, mapMovement, gridDistance, areaShapes, encounterRoom"]
    DB["server/db/\nindex.ts + schema.ts"]
    Postgres[("PostgreSQL")]
...
| `server/utils/encounterRoom.ts` | `[id]/ws.ts`, `[id].get.ts`, `[id].put.ts`, `attack.post`, `clash-initiate.post`, `clash-action.post`, `intercede-offer.post`, `intercede-claim.post`, `server/middleware/encounter-sync.ts` | All encounter pages (live map state + `encounter-updated` push for every viewer) | `[id]/ws.ts`, `[id].get.ts`, `[id].put.ts`, `EncounterMap.vue`, `server/middleware/encounter-sync.ts` | 🟡 MODERATE |
...
### General "Encounter Changed" Push (`encounter-updated`)

- Separate from the map-specific `position-patch`/`unit-moved`/`structure-damaged` messages above, the WS room now also broadcasts a generic `{ type: 'encounter-updated', encounterId }` message (`WebSocketMapMessage` union, `app/types/index.ts`) whenever any encounter action endpoint completes — covering battle log entries, `pendingRequests` (dodge/intercede/clash/recovery-check/throw-impact-attack), wound changes, etc., none of which flow through `RoomState`/`position-patch`.
- `server/utils/encounterRoom.ts` exports `notifyEncounterUpdated(encounterId)`, a thin `broadcast()` wrapper that sends this message to every connected peer for that encounter.
- **`app/server/middleware/encounter-sync.ts`** is a new global Nitro server middleware (distinct from the client route middleware in `middleware/campaign-access.ts`/`middleware/dm-access.ts` — see Dependency Graph). It runs on every request; for any `POST /api/encounters/[id]/actions/*` that finishes with a 2xx status, it extracts `encounterId` from the path and calls `notifyEncounterUpdated(encounterId)` on `res.on('finish')`. This centralizes the "an action happened, tell clients to refetch" call for all ~23 action endpoints (including any future ones) in one file, rather than each endpoint calling `notifyEncounterUpdated` individually.
- Both the GM encounter page (`encounters/[id].vue`) and the player page (`player/[tamerId].vue`) open their own WS connection to `/api/encounters/[id]/ws` (independent of, and in addition to, the connection `MapCanvas`/`EncounterMap` opens when a map view is active) and debounce-refetch (`fetchEncounter()` + related, ~150-250ms debounce) on `encounter-updated`, so battle log/pending-request/etc. changes propagate near-instantly to all viewers regardless of whether a map is open.
- Polling remains as a fallback but is lengthened from `5000`ms to `30000`ms on both pages, since the WS push now handles the normal-case responsiveness; polling only catches eventual consistency after a dropped WS message or reconnect gap. The GM page's old conditional 1s `fastRefreshInterval`/`startFastRefresh`/`stopFastRefresh` is removed — the push covers what it was working around.
...
| Throw Ally Out of AoE | Intercede option: throw an adjacent ally out of an Area Attack's cells (Clash Throw rules, positioning only — no Damage Stat hit, no secondary attack); the rescuer becomes the AoE's sole target via the existing full-swap mechanic |
