...
- **`app/server/middleware/encounter-sync.ts`** is a global Nitro server middleware (distinct from the client route middleware in `middleware/campaign-access.ts`/`middleware/dm-access.ts` — see Dependency Graph). It runs on every request; for any `POST /api/encounters/[id]/actions/*` **or `PUT /api/encounters/[id]`** that finishes with a 2xx status, it extracts `encounterId` from the path and, on `res.on('finish')`, calls `buildEncounterPayload(encounterId)` and broadcasts `encounter-state`. The PUT case (added 2026-06-13b) covers `nextTurn`/`endCombat`/all other `updateEncounter()` call sites (map placement, wound edits, etc.), which previously only updated the acting client's local state and relied on the 120s poll for everyone else; since `broadcast()` doesn't exclude the sender, the acting client also self-corrects via this push. This centralizes the "something changed, push fresh state" call for all ~23 action endpoints plus the bare encounter PUT in one file.
