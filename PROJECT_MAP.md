...
- **Skill Orders** (`rulesSettings.skillOrders: boolean`): Homebrew. When enabled, each tamer skill unlocks a Skill Option once the skill hits its threshold (4/5/6) and the skill's governing attribute has its first Special Order (3/5/6). Surfaced on tamer sheets + an in-combat action panel via `skill-order` endpoint.
